param(
    ...
    [Parameter(Mandatory = $true)][string]$GameInputIncludePath,
...
cl /nologo /std:c++17 /EHsc /W4 /WX /O2 /I"$moduleRoot\Source" /external:I"$GameInputIncludePath" /external:W0 "$PSScriptRoot\WindowsGameInputProbe.cpp" "$moduleRoot\Source\IAmSpeed\Input\InputPresentationScope.cpp" /Fe:"$outputPath\WindowsGameInputProbe.exe" /link /STACK:16777216
if errorlevel 1 exit /b %errorlevel%
"$outputPath\WindowsGameInputProbe.exe"
exit /b %errorlevel%
"@
...
$metadata = @{
    ...
    operation = 'standalone_windows_gameinput_fake_probe_no_hardware'
    exit_code = $probeExit
    compiler = 'MSVC 14.38 (vcvars_ver=14.38)'
    command_file = 'probe.cmd'
    gameinput_header_sha256 = (Get-FileHash (Join-Path $GameInputIncludePath 'GameInput.h') -Algorithm SHA256).Hash
    fixture_sha256 = (Get-FileHash (Join-Path $PSScriptRoot 'WindowsGameInputProbe.cpp') -Algorithm SHA256).Hash
}
